# Profit Generation — Phase 5 Plan (DRAFT)
...
**Phase 5 theme:** Phase 4 added the trend-following half of the system
...
## 5.1 ATR stops generalized across all strategies
...
- [ ] **5.1.2 ATR stops as default for mean-reversion strategies**
  - **Deliverable:** strategy declarations updated + tests
  - **Acceptance:** every mean-reversion strategy in `TRACKED_STRATEGIES` (RSI2 oversold, consec-bearish bounces, etc.) flips its stop method from `fixed_percent` to `atr_initial` (5.1.1) with `k=2.0` (tighter than trend's 2.5 because MR exits are quicker). Existing live/paper segregation (3.1.5) preserved — only paper-active strategies flip first; live strategies stay on existing stops until separately validated. Tests: declaration updates, no regression in existing strategy test suite.
...
- [ ] **5.1.3 Regime-aware ATR multiplier**
...
## 5.2 Fractional Kelly position sizing
...
- [ ] **5.2.1 Per-strategy Kelly calculator**
...
- [ ] **5.2.2 Fractional Kelly sizing tier**
...
## 5.3 Breakout-and-retest strategy
...
- [ ] **5.3.1 Donchian breakout-retest strategy**
  ...
  - **Acceptance:** signal logic:
    ...
    Declared `active_in_regimes=["bull", "trend"]`, `pyramidable=false` (the retest IS the entry — no add-on logic). Stop method `atr_initial` (5.1.1) with `k=1.0` (tight). Validator must PASS on a 5-year backtest before going live. Tests: signal sequencing, pending-window expiry, retest tolerance, no-chase enforcement.
...
- [ ] **5.3.2 Resistance-level breakout-retest (short side)**
...
## 5.4 Parabolic SAR exit overlay
...
- [ ] **5.4.1 SAR computation + overlay engine**
...
- [ ] **5.4.2 SAR overlay opt-in for trend strategies**
...
## 5.5 Options-based synthetic pyramiding — research milestone
...
- [ ] **5.5.1 Options synthetic pyramiding feasibility**
