...
- _(2026-06-09) Stage 3 WIRED (gate pends first live session) — `intraday-candle-continuation-15m` registered in `monitoring/config.py` (`INTRADAY_CANDLE_DECLARATIONS`: 15m, TSLA/TQQQ/AMD/COIN/SOXL = the five 15m backtest leaders; no declaration-level window — the strategy's per-bar time mask is the only time gate). Signal-only guarantee = indefinite `paused_strategies` row (source `intraday_build_stage3`); `intraday_fires` records entries, the auto_trader pause gate blocks them. `scripts/intraday_build_check.py` now asserts the Stage 3 invariant (registered + resolvable + paused). `tests/test_stage3_signal_only.py` (7 tests) proves declaration → resolver → compute → signals-row end-to-end on synthetic firing bars, plus M4 no-exit-without-ownership. Real-bar smoke: all 5 symbols compute clean on live 15m data. Full suite 2500 passed. **Gate check (next session): spot-check every `long_entry` signal vs chart — right pattern, ≥3 confirms, in-window, none 11:00–12:00 or after 14:30.**_
- _(2026-06-11) **Phantom-outcome cleanup unblocked the Stage 0/4 gate** — found ~95% of `outcomes` rows were signal-scoped phantoms (no backing fill) the orphan sweep had booked at fabricated marks, keeping the verifier permanently RED. Hardened `verify_intraday_lifecycle.py` to exclude no-fill rows (`phantom`, reports real vs phantom), guarded `reconcile_positions` sweep (quarantine as `phantom_no_fill`, never fabricate), added `phantom_no_fill` to `CLEANUP_EXIT_REASONS`, shipped `scripts/quarantine_phantom_outcomes.py` and quarantined 256 intraday phantoms. Gate is GREEN-able again: a paused signal-only strategy can no longer turn it RED. commit (phantom batch). docs/TICKET_PHANTOM_OUTCOMES.md._
- _(2026-06-11) **Stage 4 PREP shipped (flip pends single unpause at open)** — narrowed candle universe to TSLA/AMD/COIN (3 Tier-1 liquid singles; 3x ETFs TQQQ/SOXL held to Stage 7); flipped `intraday_build_check.py` to the Stage 4 invariant (must be UNPAUSED — while still paused it prints NOT READY + the exact unpause command); updated `test_stage3_signal_only` 5→3 symbols (7 pass). grace_period=True, $800 floor, 0.5x multiplier, `\TradingSystem\Intraday` task all confirmed in place. **Stage 4 go (tomorrow at open): `sh.unpause_strategy('intraday-candle-continuation-15m')`, then on first fill verify the ATR stop rests on the Alpaca book (no sub-penny reject), then EOD `verify_intraday_lifecycle --session <date>` expecting GREEN.**_
